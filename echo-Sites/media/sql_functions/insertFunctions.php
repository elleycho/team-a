...
function insertKeyword ($keyword, $resource_id)
{
  include "connectdb.php";
  ...
  if (!$result)
  {
    if (mysql_errno() == 1062)
    {
      die('ERROR: Duplicate Resource - Keyword entry for Resource: ' . $resourcename . ' [ID: ' . $resource_id . '] (Keyword: ' . $keyword . ')');
    }
    else
    {
      die('ERROR: ' . mysql_error() . '--> ERROR NO: ' . mysql_errno());
...
function insertResource ($name, $street1, $street2, $city, $zip, $program_id, $website, $purpose, $mission, $description, $eligibility, $fees)
{
  include "connectdb.php";

  $sql = "INSERT INTO resource (name, street1, street2, city, zipcode, programType_ID, website, purpose, mission, description, eligibility, fees) 
              VALUES ('".$name."','".$street1."','".$street2."','".$city."','".$zip."','".$program_id."','".$website."','".$purpose."','".$mission."','".$description."','".$eligibility."','".$fees."')";
